...
impl Renderer for LispFrameRef {
    ...
    fn draw_char_glyph_string_foreground(&mut self, s: GlyphStringRef) {
        ...
        self.canvas().display(|builder, space_and_clip, scale| {
            ...
            let glyph_instances = s.scaled_glyph_instances(builder, space_and_clip, scale);
            // draw foreground
...
impl WrGlyph for GlyphStringRef {
    ...
    fn scaled_glyph_instances(
        ...
        let instances = self.glyph_instances(builder, space_and_clip, scale);
        ...
        match type_ {
            ...
            glyph_type::COMPOSITE_GLYPH => {
                if self.automatic_composite_p() {
                    self.automatic_composite_glyph_instances(builder, space_and_clip, scale)
                } else {
    ...
    fn automatic_composite_glyph_instances(
        ...
        let mut composite_lglyph = |lglyph: LispObject, x: i32, y: i32| {
            let width = lglyph.width();
            let c = lglyph.char();
            if let Some((xoff, yoff, iwidth, iheight, image_key)) = self.font().glyph_raster_image(
                c,
                width as u16,
                (width as f32 * scale).round() as u16,
            ) {
                let yoff = yoff as i32 - (iheight as f32 / 2.0).round() as i32;
                let bounds = (x + xoff as i32, y + yoff).by(iwidth as i32, iheight as i32, scale);
                log::warn!("automatic composite glyph raster image {c:?} {bounds:?}");
                ...
                //WARN: This branch is not tested
