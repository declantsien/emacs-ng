...
;; system and process Winit-specific command line parameters before
;; creating the first Winit frame.
...
(let ((i 160))
  (while (< i 256)
    (puthash i i winit-keysym-table)
    (setq i (1+ i))))
...
(dolist
	...
	(#x20ab . ?₫)
	(#x20ac . ?€)))
  (puthash (car pair) (cdr pair) winit-keysym-table))
...
(cl-defmethod window-system-initialization (&context (window-system winit)
  ...
  (cl-assert (not winit-initialized))
