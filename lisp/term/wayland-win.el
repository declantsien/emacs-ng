...
(unless (featurep 'wayland)
  (error "%s: Loading wayland-win.el but not compiled with Wayland"
         invocation-name))
...
(defvar wayland-initialized nil
...
(cl-defmethod window-system-initialization (&context (window-system wayland)
                                            &optional display)
  "Initialize Emacs for Wayland frames and open the first connection to an
Wayland server."
  (cl-assert (not wayland-initialized))

  (wayland-open-connection (or display
			       (setq wayland-display-name (or (getenv "WAYLAND_DISPLAY" (selected-frame))
						  (getenv "WAYLAND_DISPLAY"))))
		           nil
		           ;; Exit Emacs with fatal error if this fails and we
		           ;; are the initial display.
		           (eq initial-window-system 'wayland))
...
(cl-defmethod handle-args-function (args &context (window-system wayland))
  (x-handle-args args))

(cl-defmethod frame-creation-function (params &context (window-system wayland))
  (x-create-frame-with-faces params))

;; (cl-defmethod gui-backend-set-selection (selection value
;;                                          &context (window-system wayland))
